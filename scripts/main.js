...
// Список прокси. Скрипт пробует их по очереди.
// 1. Быстрый (но может быть заблокирован в РФ).
// 2. Медленный, но надежный (AllOrigins).
const PROXIES = [
    (url) => `https://corsproxy.io/?url=${encodeURIComponent(url)}`,
    (url) => `https://api.allorigins.win/raw?url=${encodeURIComponent(url)}&disableCache=${Date.now()}`
];
...
class FoundryTubeApp extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    async _onExecuteSmartInput() {
        ...
        // Исправление ошибки консоли: очищаем и убираем фокус ДО долгого запроса
        input.value = "";
        input.blur();
    ...
    // Хелпер: перебирает прокси, пока не получит ответ
    async _fetchText(targetUrl) {
        for (const proxyGen of PROXIES) {
            try {
                ...
                console.warn(`Tube: Proxy failed for ${targetUrl}, trying next...`);
            }
        }
        return null;
    }
    ...
    async _handleImport(val) {
        if (val.includes('list=') || val.includes('playlist?')) {
            ...
            if (videos.length > 0) {
                ...
                ui.notifications.warn("Could not import playlist. Check validity or try later.");
            }
        }
    ...
    async _performSearch(q){
        const c=this.element.querySelector('#search-container');
        c.innerHTML='<div class="empty-msg"><i class="fas fa-spinner fa-spin"></i></div>';
        const r=await this._searchYoutubeScrape(q);
        if(!r.length){
            c.innerHTML='<div class="empty-msg">No results found.<br><a href="https://www.youtube.com/results?search_query='+encodeURIComponent(q)+'" target="_blank" style="text-decoration:underline">Open in Browser</a></div>';
            return;
        }
        c.innerHTML='';
        r.forEach(v=>{
            ...
            el.querySelector('.list-add').addEventListener('click',(e)=>{e.stopPropagation();this.addToPlaylist(v);ui.notifications.info(`Added: ${v.title}`);});
            el.querySelector('.list-play').addEventListener('click',(e)=>{e.stopPropagation();this.playVideoNow(v);this._onCloseSearch();this._onToggleInputMode(null,this.element.querySelector('.toggle-mode-btn'));});
            c.appendChild(el);
        });
    }
    ...
    async _searchYoutubeScrape(q){
        ...
        try {
            const t = await this._fetchText(u);
            if(!t) return [];
            const m=t.match(/ytInitialData\s*=\s*({.+?});/);
            if(!m) return [];
            const d=JSON.parse(m[1]);
            const res=[];
            const c=d.contents?.twoColumnSearchResultsRenderer?.primaryContents?.sectionListRenderer?.contents;
            if(c){
                for(const s of c){
                    if(s.itemSectionRenderer){
                        for(const i of s.itemSectionRenderer.contents){
                            if(i.videoRenderer){
                                const v=i.videoRenderer;
                                res.push({id:v.videoId,title:v.title?.runs[0]?.text||"Video",author:v.ownerText?.runs[0]?.text||"YouTube",thumb:v.thumbnail?.thumbnails[0]?.url,duration:v.lengthText?.simpleText||"Live"});
                            }
                        }
                    }
                }
